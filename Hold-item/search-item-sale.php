	...
	if(mysqli_num_rows($sql)>0)
	{
		...
		$item_inventory=mysqli_query($conn,"SELECT * FROM tblinventory WHERE productcode='".$barcode."' AND remainingqty > '0' ORDER BY inventoryId DESC");
			if(mysqli_num_rows($item_inventory)>0){
				...
				$fetchcartId=mysqli_query($conn,"SELECT * FROM tblholditem WHERE holdname='".$holdname."' AND productId='".$inventoryid."'");
		...
		$item_inventory=mysqli_query($conn,"SELECT * FROM tblinventory WHERE productcode='".$barcode."' AND remainingqty > '0' ORDER BY inventoryId DESC");
			if(mysqli_num_rows($item_inventory)>0){
				...
				$data['status']="nostock";
							...
							$gettotal=mysqli_query($conn,"SELECT * FROM tblholditem WHERE cashierId='".$cashierid."' AND holdname='".$holdname."'");
