...
$sql=mysqli_query($conn,"SELECT * FROM tblholditem   WHERE holdname='".$_GET['hold']."' AND cashierId='".$cashierid."' GROUP BY productcode ");
if(mysqli_num_rows($sql)>0){
	while($row=mysqli_fetch_array($sql))
	{
		$qty=0;
		$subtotal=0;
		$sqltotal=mysqli_query($conn,"SELECT * FROM tblholditem WHERE productcode='".$row['productcode']."' AND holdname='".$_GET['hold']."'");
