   	...
   	if(barcode.length>=3){
		 ...
		 $.ajax({
			...
			success:function(data){
				...
				$('#tblpurchase').load('addpurchase-table.php?purchaseid='+invoice);
...
//2. EDIT CLICK BUTTON
  $(".table tbody").on('click', '#btn-edit', function(e){
    ...
    var qty=row.find("td:eq(6) input").val(); 
	$.ajax({
...
//8. ON CHANGE QTY STOCK
  $(".table tbody").on('change', '#totalqty', function(e){
	...
	var invoice = $('#invoiceid').val();
    var cost=row.find("td:eq(5) input").val();//price
    var totalqty=row.find("td:eq(6) input").val();//sto 
    ...
    totalprice=row.find("td:eq(7) input").val(total_per_item.toFixed(2));//totalpricing
    totalpricehidden=row.find("td:eq(8)").text(total_per_item.toFixed(2));//totalpricinghidden
			var table = document.getElementById("tablepurchase");
		for(var i = 1; i < table.rows.length; i++)
			{
				 
			total+=parseFloat(table.rows[i].cells[8].innerHTML);
...
$('#btn-savepurchase').on('click',function(){
	...
	success:function(){
	alert('Purchase successfuly updated');
		window.location.href="../purchase/purchaselist.php";
	},
	error:function(data){
		alert(data);
	}
...
$paid;
$balance;
$sql=mysqli_query($conn,"SELECT * FROM tblinventory_merge WHERE invoiceId='".$_GET['purchaseid']."'");
...
?>
<input type="hidden" id="invoiceid" value="<?php echo $_GET['purchaseid']?>">
	 <div class="content-wrapper">
		...
		<h5 style="margin-left:2%; 	">Modify purchase item(s)</h5>
			  ...
			  <a href="#" class="dropdown-item " id="exportexcel">Export(CSV)</a>
				  ...
				  <div class="form-group" >
					 ...
					 <div class="input-group input-group-sm m-0 p-0" >
						  ...
						  <input type="text" id="reference" name="reference" class="form-control float-right" value="<?php echo $reference; ?>">
					...
					<div class="col-sm-4" style="float:left;">
					 ...
					 <div class="input-group input-group-sm m-0 p-0" >
						  ...
						  <input type="text" id="details" name="details" class="form-control float-right" value="<?php echo $details; ?>">
				  ...
				  <div class="form-group" >
					 ...
					 <div class="input-group input-group-sm m-0 p-0" >
						  ...
						  <input type="text" id="supplier" name="supplier" class="form-control float-right" value="<?php echo $supplier?>">
				  ...
				  <tr style="border:none;"> 
				    ...
				    <td id="discount_cell">
					  <div class="input-group input-group-sm m-0 p-0" >
						  ...
						  <input type="number" id="discount" name="discount" class="form-control" value="<?php echo $discount; ?>" style="font-size:1rem;"disabled>
	...
	<div class="input-group-append"  >
				    ...
				    <td id="amountpaid_cell" style="font-weight:700;">
					  <div class="input-group input-group-sm m-0 p-0" >
						  ...
						  <input type="number" id="cashamountpaid" name="cashamountpaid" class="form-control" value="<?php echo $paid; ?>" style="font-size:1rem;"disabled>
				   ...
				   <tr> 
				    ...
				    <td id="balance_cell">
					  <div class="input-group input-group-sm m-0 p-0" >
						  ...
						  <input type="number" id="balance" name="balance" class="form-control" value="<?php echo $balance; ?>" style="font-size:1rem;"disabled>
               ...
               <button type="button" id="btn-savepurchase" name="btn-savepurchase" class="col-md-2 btn btn-primary btn-save"  id="update-product">Update Purchase</button>
