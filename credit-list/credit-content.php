...
function doConfirm(msg, yesFn, noFn)
{
	...
	$(".table tbody").on('click', 'tr', function(e){
	    ...
	    var customer=row.find("td:eq(5)").text(); //getting the row column selected
		$('#credit-customer').text(customer + ' (Credit item)');
	    $('.modal-itemlist').modal("show"); 
		 $('#tblitemlist').load('fetch-solditem.php',{'invoiceid':invoiceid,});
		 
});   
	$(".table tbody").on('click', '#paid', function(e){
		e.stopImmediatePropagation();
		var row=$(this).closest("tr"); //getting the selected row of table
	    var invoiceid=row.find("td:eq(0)").text(); //getting the row column selected
	    var customer=row.find("td:eq(5)").text(); //getting the row column selected
		$('#payment-customer').text(customer+ ' (Payment)');
	    $('.modal-payment').modal("show"); 
		 $('#tblpayment').load('fetch-payment.php',{'invoiceid':invoiceid,});
...
.table tr {
    cursor: pointer;
	 ...
	 <div class="content-wrapper"  >
		...
		<h5 style="margin-left:2%; 	">Credit list </h5>
