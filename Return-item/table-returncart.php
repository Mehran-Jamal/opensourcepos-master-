...
$sql=mysqli_query($conn,"SELECT * FROM tblcartreturn   WHERE cashierId='".$cashierid."' AND invoiceId='".$_GET['invoice']."' AND qty > 0 GROUP BY productcode");
if(mysqli_num_rows($sql)>0){
	while($row=mysqli_fetch_array($sql))
	{
		$qty=0;
		$subtotal=0;
		$sqltotal=mysqli_query($conn,"SELECT * FROM tblcartreturn WHERE productcode='".$row['productcode']."' AND invoiceId='".$_GET['invoice']."'");
