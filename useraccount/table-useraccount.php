   <table   class="table table-bordered table-hover" id="tblactivity">
                 ...
                 <tr>
				  ...
				  <?php
				  $sql=mysqli_query($conn,"SELECT * FROM tbluser ORDER BY userId DESC");
            ...
            <td style="display:none;"><?php echo $row['userId'];?></td>
					...
					?>
					<span class="badge bg-primary" id="btn-status"><?php echo $row['userstatus'];?></span></td>
						<?php }?>
						<?php 
						if($row['userstatus']=="Pending"){
					?>
					<span class="badge bg-warning" id="btn-status"><?php echo $row['userstatus'];?></span></td>
						<?php }?>
